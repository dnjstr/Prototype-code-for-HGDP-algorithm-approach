...
public:
    ...
    static const int DEFAULT_JOBS = 30;
    static const int DEFAULT_MACHINES = 15;
    static const int DEFAULT_PROCESSING_TIME = 1;
    ...
    bool loadFromFile(const std::string& filename) {
        ...
        num_jobs = DEFAULT_JOBS;     // Use the constants you defined in step 3
        num_machines = DEFAULT_MACHINES;


        // Read the matrix (each row is a job, each column is an operation)
        jobs.clear();
        ...
        while (jobs_read < num_jobs && std::getline(file, line)) {
            ...
            for (int m = 0; m < num_machines; ++m) {
                int machine_sequence;
                if (!(iss >> machine_sequence)) {
                    std::cerr << "Failed to read operation " << m << " for job " << jobs_read << std::endl;
                    return false;
                }
                // Convert to (machine_id, processing_time) format
                // Assuming machine_sequence is 1-indexed, so subtract 1
                jobs[jobs_read][m] = std::make_pair(machine_sequence - 1, DEFAULT_PROCESSING_TIME);
            }
            ++jobs_read;
        }
        ...
        if (jobs_read != num_jobs) {
            std::cerr << "Expected " << num_jobs << " jobs but read " << jobs_read << std::endl;
            return false;
        }
        return true;
    }
...
const int JSSPInstance::DEFAULT_JOBS;
const int JSSPInstance::DEFAULT_MACHINES;
const int JSSPInstance::DEFAULT_PROCESSING_TIME;
...
std::vector<LiuResults> getLiuResultsDatabase() {
    ...
    // Add your TA instance results here
    results.emplace_back("TA31_MA", 0, 0);  
    results.emplace_back("TA41_MA", 0, 0);
    results.emplace_back("TA51_MA", 0, 0);
    results.emplace_back("TA61_MA", 0, 0);
    results.emplace_back("TA71_MA", 0, 0);
    return results;
}
