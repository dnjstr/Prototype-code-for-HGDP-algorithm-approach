...
std::vector<Activity> generateSkewedActivities(int numActivities, int maxStartTime, int maxDuration, int maxWeight) {
    std::vector<Activity> activities;
    std::mt19937 gen(42); // Fixed seed for consistent results
    std::uniform_int_distribution<> startDist(0, maxStartTime);
    std::uniform_int_distribution<> durationDist(1, maxDuration);
...
int main() {
    // Generate SD-3 dataset
    int numActivities = 500;  // Large-scale dataset with 500 activities
    int numParticipants = 15; // 15 participants
    int maxStartTime = 200;   // Maximum start time for activities
    int maxDuration = 30;     // Maximum duration for activities
    int maxWeight = 100;      // Maximum weight for activities
    int maxCpu = 15;          // Maximum CPU capacity
    int maxMemory = 15;       // Maximum memory capacity


    std::vector<Activity> activities = generateSkewedActivities(numActivities, maxStartTime, maxDuration, maxWeight);
    std::vector<Participant> participants = generateParticipants(numParticipants, maxCpu, maxMemory);


    // Print problem instance
    std::cout << "Problem Instance (SD-3):" << std::endl;
