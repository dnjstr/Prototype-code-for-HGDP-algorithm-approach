...
std::vector<Participant> generateParticipants(int numParticipants, int maxCpu, int maxMemory) {
    std::vector<Participant> participants;
    std::mt19937 gen(42); // Fixed seed for consistent results
    std::uniform_int_distribution<> cpuDist(3, maxCpu);    // Smaller CPU capacity
    std::uniform_int_distribution<> memoryDist(3, maxMemory); // Smaller memory capacity
...
int main() {
    ...
    int numActivities = 50;   // Small-scale dataset with 50 activities
    int numParticipants = 5;  // 5 participants
    int maxStartTime = 50;    // Maximum start time for activities
    int maxDuration = 10;     // Maximum duration for activities
    int maxWeight = 20;       // Maximum weight for activities
    int maxCpu = 5;           // Maximum CPU capacity
    int maxMemory = 5;        // Maximum memory capacity


    std::vector<Activity> activities = generateUniformActivities(numActivities, maxStartTime, maxDuration, maxWeight);
    std::vector<Participant> participants = generateParticipants(numParticipants, maxCpu, maxMemory);


    // Print problem instance
    std::cout << "Problem Instance (SD-1):" << std::endl;
